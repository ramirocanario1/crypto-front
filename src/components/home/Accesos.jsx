...
export default function Accesos() {
    return (
      <HomeSection title={'📲 Accesos'} description={'Accede a las principales funciones de la app.'}>
        <nav className='flex gap-3 overflow-x-auto p-2'>
          ...
          <Acceso to='/perfil' icon={<FaUserAlt className='w-full h-full text-yellow-400'/>} label='Perfil'/>
  ...
  function Acceso({icon, label='', to='#'}) {
    return (
      <Link to={to} className={`bg-gray-800 p-4 rounded-md h-20 w-20 flex gap-1 flex-col items-center justify-center flex-shrink-0`}>
        {icon}
